...
'''
import subprocess
#subprocess.Popen(['bash','./setup_with_check_in.sh'])
print('Start shell script')
# Setup Environment
process = subprocess.Popen(['bash','./setup_in.sh'])
process.wait()
...
# LIBRARIES SUGGESTED BY TENSORFLOW BUT NOT USED
# import zipfile
...
################################################################################
# CLASS DETECTOR TO BE USED
################################################################################


class Detector():

    # Check to see if the Tensorflow version is high enough
    ...
    def run_inference_for_single_image(self,image,showFigs=False):
      with self.detection_graph.as_default():
...
print("Code Ended.")
######################################################################
# INSTRUCTIONS ON USING THIS CLASS
'''
myDetector = Detector()
img = cv2.imread('/home/nickshu/Desktop/airsim_testImg.jpg',1)
res = myDetector.run_inference_for_single_image(img,showFigs=True)
plt.show()
'''
