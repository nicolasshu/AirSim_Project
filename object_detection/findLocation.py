...
print(cameraTypeMap[cameraType1])
print(cameraTypeMap[cameraType2])
print(cameraTypeMap[cameraType3])
...
################################################################################
# USE Detector() TO IDENTIFY LOCATION OF CAR
################################################################################
...
# CHECK COLOR OF BLOB
################################################################################
# Assuming we have [x_coord,y_coord]

# Get the HSV Image
png_scene_hsv = cv2.cvtColor(png_scene, cv2.COLOR_BGR2HSV)
targetHSV = png_scene_hsv[y_coord][x_coord]
...
# Create the Mask
mask = cv.inRange(png_scene_hsv, lower_Hue, upper_Hue)

# Create the Masked Image Result
finalResult = cv.bitwise_and(png_scene_hsv, png_scene_hsv, mask= mask)
...
################################################################################
# SIMPLE ALGORITHM TO FIND LOCATION
################################################################################
